...
param (
    ...
    [Parameter()][String]$EnterpriseScaleToolsModuleDirectory = "$PWD/Enterprise-Scale-Tools",
...
# module.
$esltModulePath = "$EnterpriseScaleToolsModuleDirectory/EnterpriseScaleLibraryTools.psm1"
...
if ($UseCacheFromModule -and (Test-Path "$EnterpriseScaleToolsModuleDirectory/ProviderApiVersions.zip")) {
    Write-Host "Pre-loading ProviderApiVersions from saved cache."
    Invoke-UseCacheFromModule($EnterpriseScaleToolsModuleDirectory)
}
